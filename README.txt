...
-(DONE) It should present a custom form with 3 text inputs. We want you to use text inputs rather than radio buttons or menus so that you can practice validation.
-(DONE) One input should be a shift value, one should be a direction, and the third should be the phrase to be encrypted.
-(PAGE REDIRECT IN PLACE) Then you should redirect to a second page to show the result - the encoded phrase. Here are a couple examples of input and output.
...
-(DONE) Your final result should be in all lowercase.
...
-Add another page where the user could enter an encrypted phrase with its shift value and direction to have it translated back into english.
-Include a way to generate the shift direction and amount randomly. Your user should still be able to enter their own values too.
-Store the shift direction and amount as session variables. Then add another page where a user could enter the encrypted phrase and have it translated back into english using the stored values rather than having to enter the direction and amount manually.
